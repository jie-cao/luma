...
// Bone matrices (skinning matrices)
...
struct VSInput {
    ...
    float2 uv : TEXCOORD;
    float3 color : COLOR;
    uint4 boneIndices : BLENDINDICES;  // Up to 4 bones per vertex
    float4 boneWeights : BLENDWEIGHT;  // Weights for each bone
};
...
float3 skinPosition(float3 pos, uint4 boneIdx, float4 weights) {
    float3 result = float3(0, 0, 0);
    
    [unroll]
    for (int i = 0; i < 4; i++) {
        if (weights[i] > 0.0) {
            float4x4 bone = boneMatrices[boneIdx[i]];
            result += mul(bone, float4(pos, 1.0)).xyz * weights[i];
        }
    }
...
float3 skinDirection(float3 dir, uint4 boneIdx, float4 weights) {
    float3 result = float3(0, 0, 0);
    
    [unroll]
    for (int i = 0; i < 4; i++) {
        if (weights[i] > 0.0) {
            float3x3 bone = (float3x3)boneMatrices[boneIdx[i]];
            result += mul(bone, dir) * weights[i];
        }
    }
...
PSInput VSMain(VSInput input) {
    ...
    float3 skinnedNormal = skinDirection(input.normal, input.boneIndices, input.boneWeights);
    float3 skinnedTangent = skinDirection(input.tangent.xyz, input.boneIndices, input.boneWeights);
    
    // Transform to world space
    float4 worldPos = mul(world, float4(skinnedPos, 1.0));
    output.position = mul(worldViewProj, float4(skinnedPos, 1.0));
    output.worldPos = worldPos.xyz;
    output.normal = normalize(mul((float3x3)world, skinnedNormal));
    output.tangent = normalize(mul((float3x3)world, skinnedTangent));
...
// Helper functions (same as pbr.hlsl)
