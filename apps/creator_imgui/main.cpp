...
struct Application {
    ...
    luma::SceneManager sceneManager;
    
    // Legacy single model (for backward compatibility during transition)
    luma::LoadedModel legacyModel;
    bool useLegacyMode = false;  // true = single model, false = scene mode
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_MOUSEMOVE:
        if (g_app.viewport.cameraMode != luma::CameraMode::None) {
            // Compute scene bounds for camera control
            float sceneCenter[3];
            float sceneRadius;
            g_app.sceneManager.computeSceneBounds(sceneCenter, sceneRadius);
            g_app.viewport.onMouseMove((float)GET_X_LPARAM(lParam), (float)GET_Y_LPARAM(lParam), sceneRadius);
        }
        return 0;
    ...
    case WM_MOUSEWHEEL:
        if (!imguiWantsMouse) {
            ...
            float sceneRadius;
            g_app.sceneManager.computeSceneBounds(sceneCenter, sceneRadius);
            g_app.viewport.onMouseWheel(delta, sceneRadius);
        }
        return 0;
...
static void RenderUI() {
    ...
    if (menuAction == "__OPEN_DIALOG__") {
        ...
        if (!path.empty()) {
            // Add to scene
            uint32_t nodeId = g_app.sceneManager.createNodeFromModel(path, g_app.renderer);
            if (nodeId != 0) {
                g_app.sceneManager.getScene().setSelectedNode(nodeId);
                // Update camera target to scene center
                float center[3];
                float radius;
                g_app.sceneManager.computeSceneBounds(center, radius);
                g_app.viewport.camera.targetX = center[0];
                g_app.viewport.camera.targetY = center[1];
                g_app.viewport.camera.targetZ = center[2];
            }
        }
    }
    
    // Check for "Add Model" from hierarchy panel
    if (ImGui::GetClipboardText() && strcmp(ImGui::GetClipboardText(), "__ADD_MODEL__") == 0) {
        ImGui::SetClipboardText("");  // Clear
        std::string path = OpenFileDialog();
        if (!path.empty()) {
            uint32_t nodeId = g_app.sceneManager.createNodeFromModel(path, g_app.renderer);
            if (nodeId != 0) {
    ...
    // Panels
    if (g_app.useLegacyMode) {
        if (luma::ui::drawModelPanel(g_app.legacyModel)) {
            std::string path = OpenFileDialog();
    ...
    // Scene panels
    luma::ui::drawSceneHierarchy(g_app.sceneManager);
    luma::ui::drawPropertiesPanel(g_app.sceneManager);
...
int main() {
    ...
    // Add to scene
    luma::AssetID defaultAssetId;
    g_app.sceneManager.loadModel("", g_app.renderer, defaultAssetId);  // This won't work, let's create it manually
    // Actually, let's just create a node with a simple model reference
    // For now, we'll start with an empty scene and let users add models
    ...
    while (!g_app.shouldQuit) {
        ...
        // Handle pending model load (legacy mode)
        if (!g_app.pendingModelPath.empty()) {
            g_app.renderer.loadModel(g_app.pendingModelPath, g_app.legacyModel);
        ...
        if (g_app.useLegacyMode) {
            g_app.viewport.render(g_app.renderer, g_app.legacyModel);
        } else {
            g_app.viewport.renderScene(g_app.renderer, g_app.sceneManager);
        }
